<template>
  <div class="orderCreate relative van-safe-area-botton !pb-[0] orderCreatewide van-safe-area-top">
    <van-nav-bar
      class="processActive wideOrderCreatePath_nav van-safe-area-top !bg-[var(--van-button-primary-background)]"
      title=""
      left-arrow
      @click-left="onClickLeft"
      :class="processActive == 4 ? 'processActive' : ''"
    />
    <!-- :class="[7].includes(BuyOrderDetail.status) ? 'overflow_a' : ''" -->
    <!-- <nav-bar :mode="processActive == 4 ? 'white' : 'transparent'" @click-left="onClickLeft" /> -->
    <!-- <nav-bar mode="white" /> -->
    <div class="wideOrderCreatePath overflow_a">
      <div
        class="top_wrop"
      ...
      <div
        ...
        v-if="
          ![2, 4, 5, 8, 9, 10].includes(BuyOrderDetail.status) && processActive !== 4 && !SellOrder
        "
      >
      ...
      <div class="FcontentWrap px-[0px]">
        <!-- :class="isiOS ? 'pb-[75px]' : ''" -->
        <div class="clearfix">
          <div class="Fcontent van-safe-area-bottom !pr-[9px] left">
            <div
                ...
                <div class="clearfix">
                  <div class="left w-[60%]">
                    ...
                    >
                      {{ seller ? '在' : '等待卖家确认交易，' }}
                      <span class="O_red">
                        {{ secondsToMinutesWithSeconds(BuyOrderDetail.time_second) }}
                      </span>
                      后交易将取消
                    </div>
              ...
              >
                <div class="px-[15px]">
                  ...
                  >
                    ...
                    <span class="payTitle">
                      银行卡支付
                      <span class="text-[red]">支持 支付宝/网银/微信 转账</span>
                    </span>
                  </div>
                  <div
                  ...
                  <div class="O_count">
                    {{ seller ? '付款' : '收款'
                      ...
                      ['BANK', 'SZRMB'].includes(BuyOrderDetail.coin_protocol) && seller
                        ? '后4位'
                        : ''
                    }}
                    <span class="O_nums">
                  ...
                  <div
                    class="O_amount clearfix mb-[20px]"
                  ...
                  >
                    ...
                    <div class="O_nums">
                      <van-image
                        class="payImg1"
                        lazy-loadss
                        @click="showImagePreview([pay_pic])"
                        :src="pay_pic"
                      />
                    </div>
                  </div>
          ...
          <!-- right -->
          <div class="Fcontent right !pt-[170px] !pl-[9px]">
            <!-- 上传付款凭证 -->
            ...
            >
              <p class="text-[25px] my-[26px]">
                ...
                <van-popover
                  ...
                  placement="top"
                  className="sell_popover1  van-popup van-popover van-popover--light "
                >
              ...
              />
              <div
                class="toips_uploader right text-[24px]"
            ...
            <div
              v-if="
                ...
                onlineCharge &&
                !seller &&
                [3, 6, 7, 8].includes(BuyOrderDetail.status)
              "
              class="O_orderInfo O_pay clearfix"
              :class="[7, 6, 8].includes(BuyOrderDetail.status) ? 'bg_info' : '!bg-[#fff]'"
            >
              <div class="mt-[10px] h-[50px] leading-[50px] my-[10px]">
                <van-field
              ...
              <div class="h-[50px] leading-[50px] my-[15px]">
                <!-- v-Replaceinput -->
                <van-field
                  :input-align="[7, 6, 8].includes(BuyOrderDetail.status) ? 'right' : 'left'"
              ...
              <div
                class="h-[50px] leading-[50px] my-[15px] !mb-[0]"
                v-if="BuyOrderDetail.coin_protocol == 'BANK'"
              >
                <van-field
                  :input-align="[7, 6, 8].includes(BuyOrderDetail.status) ? 'right' : 'left'"
              ...
              <p
                v-if="
                  BuyOrderDetail.coin_protocol == 'BANK' &&
                  ![7, 6, 8].includes(BuyOrderDetail.status)
                "
                class="mt-[30px] text-center"
              >
                注：银行名称必须包含银行、邮政、信用社的其中一个
              </p>
            </div>
            ...
            <div
              v-if="
                !onlineCharge &&
                [8].includes(BuyOrderDetail.status) &&
                processActive == 3 &&
                !SellOrder
              "
        ...
        <!-- 按钮 -->
        <div class="orderButtonWrap mt-[30px]">
          <div
          ...
          >
            <van-button
              ...
              class="orderOperation !bg-[#fff] !text-[var(--van-button-primary-background)]"
            >
              {{ '联系客服' }}
            </van-button>
            <div
          ...
          取消挂单
            ...
            <van-button
              type="primary"
              v-if="
                ![2, 4, 5, 9, 10].includes(BuyOrderDetail.status) && processActive == 2 && !seller
              "
              class="orderOperation"
              @click="
                ;+is_upbuyimg_qztj && (!!!fileList[0] || !!!fileList[0].UploadURL)
                  ...
                  : verify_input(pay_cardid, pay_realname_backUp, pay_bankname) &&
                    BuyerUpdateTransInfo({
                      ...
                      ;(payimg = false), (fileList = []), getBuyOrderDetail(BuyOrderDetail.order_id)
                    })
              "
            >
              我已付款，通知卖家
            ...
            <van-button
              type="primary"
              v-if="
                !SellOrder && processActive == 3 && seller && [7].includes(BuyOrderDetail.status)
              "
              class="orderOperation"
              @click="
                authStore.userInfo.is_set_paypassword
    ...
    >
      ...
      <van-uploader
        ...
        :max-size="10 * 1000 * 1024"
        @oversize="onOversize"
        class="O_uploader left !mr-[10px]"
      />
      <div class="appeal !right !mt-[0]">
        请勿发送与交易无关的内容，如有发现有广告，淫秽等内容，会进行封号处理，后果自负。
      </div>
      <div class="flex_z w-[100%]">
        <van-button
          type="primary"
          color="#828282"
          class="margr20"
...
import { FieldType, showImagePreview, showToast } from 'vant'
    ...
    if (!!!pay_bankname) {
      ...
      if (
        !(
          ...
          pay_bankname.endsWith('农信') ||
          pay_bankname.endsWith('信用社')
        )
      ) {
        showInfoToast('请以银行/信用社/农信结尾')
        return false
      }
    }
  }
